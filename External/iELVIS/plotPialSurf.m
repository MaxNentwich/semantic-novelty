...
try
    ...
    if overlayParcellation,
        ...
        elseif strcmpi(overlayParcellation,'PALS')
            annotFname=fullfile(labelFolder,[side 'h.PALS_B12_Visuotopic.annot']); % PALS
            [averts,albl,actbl]=read_annotation(annotFname);
%             actbl.table(end,1:3)=255*[1 1 1]*.7; %make medial wall the same shade of grey as functional plots
        ...
        elseif exist(overlayParcellation,'file')
            ...
            %  actbl.table(43,1:3)=255*[1 1 1]*.7; %make medial wall the same shade of grey as functional plots
        elseif strcmpi(overlayParcellation,'MT')
            annotFname=fullfile(labelFolder,[side 'h.aparc.annot']); %Desikan-Killiany 36 area atlas
            [~,albl,actbl]=read_annotation(annotFname);
            actbl.table(1,1:3)=255*[1 1 1]*.7; %make medial wall the same shade of grey as functional plots
            actbl.table(1,5) = actbl.table(1,1) + actbl.table(1,2)*2^8 + actbl.table(1,3)*2^16;
            ...
            for v = 1:length(rh)
                dist = sum((cort.vert - rh(v, 2:4)).^2, 2);
                [~, idx_rh(v)] = min(dist);
            end
            
            for v = 1:length(lh)
                dist = sum((cort.vert - lh(v, 2:4)).^2, 2);
                [~, idx_lh(v)] = min(dist);
            end
                
            if strcmp(hemi_flag, 'omni')
                idx = [idx_lh, idx_rh];
            elseif strcmp(hemi_flag, 'r')
                idx = idx_rh;
            elseif strcmp(hemi_flag, 'l')
                idx = idx_lh;
            end

            % Update table
            actbl.numEntries = 2;
            actbl.orig_tab = '/media/max/RedPassport/ECoGData/Tobii/FullAnatomy/Updated/fsaverage/label/lh.MT.label';
            actbl.struct_names{2} = 'MT';
            actbl.struct_names(3:end) = [];
            actbl.table(2,1:3) = [215, 45, 0];      % red  
            actbl.table(2,5) = actbl.table(2,1) + actbl.table(2,2)*2^8 + actbl.table(2,3)*2^16;
            actbl.table(3:end, :) = [];
            
            % Set vertices in MT to red
            albl(idx) = actbl.table(2,5);
            % All others to grey
            albl(setdiff(1:length(albl), idx)) = actbl.table(1,5);
