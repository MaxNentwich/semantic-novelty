...
function plot_saccades_novelty(options)
    ...
    for b = 1:length(options.band_select)
        ...
        if exist(sprintf('%s/%s', stats_dir, vid_file), 'file') ~= 0
            ...
            loc_high = localize_elecs_bipolar(labels_high, options_main.atlas);
            loc_low = localize_elecs_bipolar(labels_low, options_main.atlas);
            ...
            n_lobes_high = zeros(size(regions));
            n_lobes_low = zeros(size(regions));
            ...
            for l = 1:length(regions)
                ...
                n_lobes_high(l) = round(sum(mean([cellfun(@(C) contains(C, regions{l}), loc_high(:,1)), ...
                    cellfun(@(C) contains(C, regions{l}), loc_high(:,2))],2)));    
                n_lobes_low(l) = round(sum(mean([cellfun(@(C) contains(C, regions{l}), loc_low(:,1)), ...
                    cellfun(@(C) contains(C, regions{l}), loc_low(:,2))],2)));    
            ...
            %% Figures 
            n_stacked = [n_lobes_high, n_lobes_shared, n_lobes_low]./n_lobes_all;
            ...
            if exist(file_ratio_total, 'file') == 0
                ...
                % Plot the errors
                eb_high = errorbar(bar_total.XEndPoints, bar_total.YEndPoints, 1.96*se_prop(:,1), 1.96*se_prop(:,1));    
                eb_high.Color = [0 0 0];                            
            ...
            if exist(file_ratio_conditions, 'file') == 0
                ...
                if strcmp(options_main.atlas, 'lobes')
                    ...
                    legend({'High Novelty', 'Both', 'Low Novelty'}, 'Position', [0.668,0.024,0.314,0.155])
                ...
                elseif strcmp(options_main.atlas, 'AparcAseg_Atlas')
                    ...
                    legend({'High Novelty', 'Both', 'Low Novelty'}, 'Position', [0.025, 0.02, 0.108, 0.105])
            ...
            % Colors
            color_high = [0.15 0.8 1];
            color_low = [0 0 0.3];
            color_shared = [0.075 0.4 0.65];
            ...
            options.fig_features.file_name = 'saccade_novelty_electrode_location';
