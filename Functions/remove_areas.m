...
function [regions, jaccard_dist, jaccard_shuff_median, n_lobes_all_sig, n_lobes_c1, n_lobes_c2, n_lobes_c3, n_lobes_all] = ...
    remove_areas(options, regions, jaccard_dist, jaccard_shuff_median, n_lobes_all_sig, n_lobes_c1, n_lobes_c2, n_lobes_c3, n_lobes_all)

    if ~options.inlude_unknown  

        idx_remove = cellfun(@(C) strcmp(C, 'Unknown'), regions);
        jaccard_dist(:, idx_remove) = [];
        jaccard_shuff_median(:, idx_remove) = [];
        regions(idx_remove) = [];
