 ...
 function saccade_novelty_frequency(options)
    ...
    if exist(file_timing, 'file') == 0
        ...
        for b = 1:length(options.band_select)
            ...
            figure
            hold on
            histogram(log(time_after_cuts_high), 75)
            histogram(log(time_after_cuts_low), 75)

            xlabel('Time after Cuts [log(s)]')
            ylabel('# of Saccades')
            ...
            diff_time = (nanmedian(time_after_cuts_high) - nanmedian(time_after_cuts_low))/fr(1);
