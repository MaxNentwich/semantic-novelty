...
addpath(genpath(sprintf('%s/Data', options.w_dir)))                                 % Data not related to patients
addpath(genpath(sprintf('%s/Organize', options.w_dir)))                             % Tables and variables for organization
addpath(genpath(sprintf('%s/Packages/TDTMatlabSDK/TDTSDK', options.w_dir)))           % Code to load TDT files
addpath(genpath(sprintf('%s/Packages/chebfun-master/', options.w_dir)))               % For image compression 
addpath(genpath(sprintf('%s/Packages/iELVis-master/', options.w_dir)))                % For plotting electrodes
addpath(genpath(sprintf('%s/Packages/Violinplot-Matlab-master/', options.w_dir)))     % For violin plots
addpath(genpath(sprintf('%s/Packages/CBIG-master', options.w_dir)))                   % Clustering of distance matrices

% Some of Stephans code to handle iEEG data
addpath(sprintf('%s/Packages/EPIPROC-master/EXTERNALS/eeg_toolbox_v1_3_2/stats', options.w_dir))
addpath(sprintf('%s/Packages/EPIPROC-master/EXTERNALS/Psychtoolbox', options.w_dir))
addpath(sprintf('%s/Packages/EPIPROC-master/proc_ecog/channel_handling', options.w_dir))
addpath(sprintf('%s/Packages/EPIPROC-master/matlab_helpers/admin', options.w_dir))
addpath(sprintf('%s/Packages/EPIPROC-master/matlab_helpers/text_processing', options.w_dir))
addpath(sprintf('%s/Packages/EPIPROC-master/EXTERNALS/eeg_toolbox_v1_3_2/events', options.w_dir))
